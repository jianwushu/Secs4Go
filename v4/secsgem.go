...
// Send 发送消息
// reply 为 nil 表示无需回复；reply 不为 nil 表示需要回复，reply 会在收到回复后填充
func (s *SecsGem) Send(msg *Message, reply *Message) error {
	// 编码 Item
	var itemData []byte
	if msg.Item != nil {
		var err error
	...
	if reply != nil {
		...
		// 解析回复
		if len(replyData) > 0 {
			parsed, _ := ParseMessage(header, replyData, s.transport)
			reply.Stream = parsed.Stream
			reply.Function = parsed.Function
			reply.WBit = parsed.WBit
			reply.SystemBytes = parsed.SystemBytes
			reply.Item = parsed.Item
		}
	} else {
		// 无需回复
...
func (s *SecsGem) sendAndWait(frameData []byte, header HSMSHeader) ([]byte, error) {
	...
	defer s.pendingReplies.Delete(systemBytes)
...
func (s *SecsGem) logReceivedData(msg *Message) {
	...
	var itemData []byte
	if msg.Item != nil {
		itemData, _ = EncodeItem(msg.Item)
	}
	frameData := BuildCompleteFrame(header, itemData)
...
func (s *SecsGem) SendDefaultReply(msg *Message) {
	if msg.WBit {
		...
		// header := BuildDataHeader(s.config.DeviceID, reply.Stream, reply.Function, false, reply.SystemBytes)
		// frameData := BuildCompleteFrame(header, reply.Item)
		// s.logger.Info(">>> Send S%dF%d(W=false, SysBytes=%d)\n%s\n%s", reply.Stream, reply.Function, reply.SystemBytes, FormatHexData(frameData), FormatSML(reply.Item))
		// s.transport.Send(frameData)
	}
